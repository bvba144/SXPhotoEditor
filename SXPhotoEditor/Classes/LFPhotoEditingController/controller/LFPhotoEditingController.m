...
- (void)configCustomNaviBar
{
    ...
    CGFloat editOkWidth = [[NSBundle LFME_localizedStringForKey:@"_LFME_sendButtonTitle"] boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:font} context:nil].size.width + 30;
    ...
    [_edit_finishButton setTitle:[NSBundle LFME_localizedStringForKey:@"_LFME_sendButtonTitle"] forState:UIControlStateNormal];
    ...
    void (^finishImage)(UIImage *) = ^(UIImage *image){
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if ([weakSelf.delegate respondsToSelector:@selector(lf_PhotoEditingController:didFinishPhotoEdit:caption:)]) {
                    [weakSelf.delegate lf_PhotoEditingController:self didFinishPhotoEdit:photoEdit caption:_captionTextField.text];
                }
    ...
    if (_edit_sticker_toolBar == nil) {
        ...
        if (self.stickerBarCacheResource) {
            ...
            if (stickerContents == nil) {
                stickerContents = @[
                    [LFStickerContent stickerContentWithTitle:[NSBundle LFME_localizedStringForKey:@"_LFME_defaultButtonTitle"] contents:@[LFStickerContentDefaultSticker]],
                    [LFStickerContent stickerContentWithTitle:[NSBundle LFME_localizedStringForKey:@"_LFME_photoAlbumButtonTitle"] contents:@[LFStickerContentAllAlbum]]
                ];
            }
